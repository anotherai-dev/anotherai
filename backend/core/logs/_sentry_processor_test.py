...
def test_sentry_disabled():
    ...
    event_dict = processor(None, Mock(), {"level": "error"})
    assert event_dict.get("sentry") != "sent"


def test_sentry_skip():
    processor = SentryProcessor(verbose=True)
    event_dict = processor(None, Mock(), {"sentry_skip": True, "level": "error"})
...
def test_sentry_log(sentry_events, level, level_value):
    ...
    processor(None, Mock(), event_data)
...
def test_sentry_log_only_errors(sentry_events, level):
    ...
    event_dict = processor_only_errors(
        None,
        Mock(),
        {"level": level, "event": level + " message"},
    )
...
def test_sentry_log_specific_keys_as_tags(sentry_events, level):
    ...
    assert_event_dict(event_data, sentry_events)
    assert sentry_events[0]["tags"] == {k: event_data[k] for k in tag_keys if k in event_data}
...
def test_sentry_get_logger_name():
    ...
    assert SentryProcessor._get_logger_name(logger=None, event_dict=event_data) == "EventLogger"
    ...
    assert SentryProcessor._get_logger_name(logger=None, event_dict=event_data) == "RecordLogger"
...
def test_sentry_json_respects_global_with_locals_option_no_locals(sentry_events):
    ...
    try:
        ...
        processor(None, Mock(), {"level": "error", "exc_info": True})
...
def test_breadcrumbs_with_additional_data(sentry_events):
    processor = SentryProcessor(verbose=True)
    processor(None, Mock(), {**base_info_log, "foo": "bar"})
    processor(None, Mock(), base_error_log)
...
def test_breadcrumbs_with_no_additional_data(sentry_events):
    processor = SentryProcessor(verbose=True)
    processor(None, Mock(), base_info_log)
    processor(None, Mock(), base_error_log)
