# from https://github.com/kiwicom/structlog-sentry
...
from sentry_sdk.integrations.logging import _IGNORED_LOGGERS  # pyright: ignore[reportPrivateUsage]
...
def _figure_out_exc_info(v: Any) -> ExcInfo:
    ...
    if isinstance(v, tuple):
        return v
    if v:
        return sys.exc_info()  # pyright: ignore[reportReturnType]
...
class SentryProcessor:
    ...
    def _get_event_and_hint(self, event_dict: EventDict) -> tuple["Event", dict[str, Any]]:
        ...
        event["message"] = event_dict.get("event") or "no message"
        event["level"] = event_dict.get("level") or "info"
    ...
    def _get_breadcrumb_and_hint(self, event_dict: EventDict) -> tuple[dict[str, Any], dict[str, Any]]:
