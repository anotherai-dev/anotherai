...
class HTTPXProviderBase[ProviderConfigVar: ProviderConfigInterface, ProviderRequestVar](
    AbstractProvider[ProviderConfigVar, ProviderRequestVar],
    ABC,
):
    ...
    def _handle_error_status_code(self, response: Response):
        match response.status_code:
            case 401 | 403:
                err = InvalidProviderConfigError(f"Config {self._config_id} seems invalid", response=response)
                if not self._config_id:
                    ...
                    self.logger.exception("Provider request timed out", response=response.text, exc_info=err)
                raise err
            case 402:
    ...
    def _wrap_errors(
        ...
        try:
            ...
            # if no exception is raised, then it's an unknown error
            raise self._unknown_error(e.response) from e
            ...
            self._get_logger().warning("Provider request timed out", request_url=e.request.url, exc_info=e)
            raise ReadTimeOutError(retry=True, retry_after=10) from e
        except httpx.RemoteProtocolError as e:
            raise ProviderInternalError(
                msg="Provider has disconnected without sending a response.",
                retry_after=10,
            ) from e
        finally:
            if finally_block:
    ...
    def _initial_usage(cls, messages: list[Message]) -> LLMUsage:
        ...
        for m in messages:
            for content in m.content:
                if content.file:
                    if content.file.is_image or content.file.is_pdf:
                        image_count += 1
                    if content.file.is_audio:
                        has_audio = True
