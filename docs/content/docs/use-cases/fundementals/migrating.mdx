...
If you want to keep your agent as is in code with minimal changes, you can ask Claude Code to do the following:
...
If your agent isn't currently using [input variables](/integrations/openai#1-input-variables) or [structured output](/integrations/openai#2-structured-outputs), you can ask Claude Code to update your agent's code to use them during the migration:
...
Send the following prompt to Claude Code (or your preferred AI coding agent):
```
...
Claude Code will take care of the steps to migrate the agent to AnotherAI including: 
...
We have a zero lock-in promise. If you're not happy with AnotherAI at any point, we make it easy to migrate away. Learn more [here](/getting-started#anotherais-zero-lock-in-promise).
