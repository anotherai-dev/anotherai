...
For image agents, we recommend including a few examples of the images you want to use as input to the agent in your initial prompt. You can learn more about how to work with image test data [here](/use-cases/image-agents#data-for-image-experiments).

```
Create an AnotherAI agent that provides a list of detected foods and their approximate 
calorie count from a given image. Use the content of @food-analysis-dataset.json as inputs 
to test the agent with an AnotherAI experiment.
```
...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie 
count from a given image where the average response time is less than 4 seconds. Use the content of 
@food-analysis-dataset.json as inputs to test the agent with an AnotherAI experiment.
```
...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie 
count from a given image that costs less than $5 for 1000 requests. Use the content of 
@food-analysis-dataset.json as inputs to test the agent with an AnotherAI experiment.
```
...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie 
count from a given image and include a customer_id metadata key. Use the content of 
@food-analysis-dataset.json as inputs to test the agent with an AnotherAI experiment.
```
...
```
How much is anotherai/agent/food-analyzer costing me this month?
```
...
![Fast models experiment](/images/fast-models-experiment-food-analyzer.png)

For the prompt requesting a cost-effective agent:

![Cost-effective models experiment](/images/cost-optimization-experiment-food-analyzer.png)
...
### Data for Image Experiments

For text-based agents, it's quite simple for your AI assistant to generate data for you, however for image-based agents, AI assistants are inconsistent about generating valid image URLs as input. Therefore, to produce useful results, we strongly recommend that you use your own images for testing. There are a couple ways to do this:

#### Using a Dataset

A dataset is a collection of images - either local files or public URLs - stored in a structured format (like a folder of images, or a JSON file with image URLs and optional expected outputs) that you can use to test your agent. Datasets can contain expected outputs, but do not need to. 
...
- They allow you to make a large number of images easily available in one file or folder, so you don't need to reference each image individually to your AI assistant each time you want to test your agent. 
...
Example dataset with images only:
```json
[
  {
  ...
  {
    "image_url": "https://example.com/images/photo2.jpg"
  },
  {
    "image_url": "https://example.com/images/photo3.jpg"
  }
]
```
...
If you have a small number of images to test that you can also upload them individually to your AI assistant. Since images uploaded this way aren't stored in a persistent dataset, this approach is best suited for ad-hoc testing rather than systematic experimentation.  While any completions created using these images will be stored in AnotherAI and can be reused in future experiments, if you wanted to test any specific images again at a later date, you would need to provide links to the specific completions that used those images. 

Refer to the next section below to learn how to upload and reference images properly for use in AnotherAI.

#### How to reference images properly for use in AnotherAI
When it comes to using image data as input, the inputs can be either local files (ex. `/Users/username/documents/george-washington.png`) or public URLs (ex. `https://cdn.britannica.com/43/173043-050-957816CE/oil-George-Washington-canvas-Gilbert-Stuart-Washington.jpg`). Here is the process for using each type of image as input:

**Public URLs**

If the images you want to use are available via public URLs and you're building your dataset in an IDE like Cursor:
...
2. Paste the image URL(s) directly into your AI assistant's chat and ask them to add it to your dataset.

```
Add https://cdn.britannica.com/43/173043-050-957816CE/oil-George-Washington-canvas-Gilbert-Stuart-Washington.jpg 
as an input to my us-presidents.json dataset.
```
...
```
Add https://cdn.britannica.com/43/173043-050-957816CE/oil-George-Washington-canvas-Gilbert-Stuart-Washington.jpg 
to my us-presidents.json dataset as input. 
Then view the content of this URL, write a description of the image and add it to the dataset 
as the input's expected output.
```

**Local Images**

If the images you want to use are available via local files and you're building your dataset in an IDE like Cursor:
1. Create a folder in your for your dataset in your project. 
2. Locate the local files on your computer that you want to use as inputs
3. Drag and drop the images directly into the folder you created for your dataset
...
4. From there, you can reference the image in your AI assistant chat freely (either by `@[file_name]` or by dragging and dropping it into the chatbox). Your AI assistant will be able to access the image to provide a ground-truth expected output, or add it as an input to an AnotherAI experiment. 

```
Create a new experiment of anotherai/agent/image-identifier and find the fastest model 
that can correctly identify this image:
/Developer/anotherai/datasets/us-presidents/george-washington.png
...
```
Create a new experiment of anotherai/agent/image-identifier and use the content 
of @us-presidents.json as inputs to the experiment.
```
### Prompts
...
```
Look at the prompt of anotherai/agent/food-analyzer and create an experiment in AnotherAI 
that compares the current prompt with a new prompt that ensures the food is listed from largest 
to smallest detected quantity. Use the content of @food-analysis-dataset.json as inputs to 
test the agent with an AnotherAI experiment.
```
...
![Terminal output showing experiment creation](/images/experiment-prompt-enhancement-terminal-food-analyzer.png)

You can use the provided URL to view the results in the AnotherAI web app to perform manual analysis of the results.

![Experiment prompts comparison](/images/experiment-prompts-comparison-food-analyzer.png)
...
but still maintains the clear and accurate analysis as my current model. Use the content of 
@food-analysis-dataset.json as inputs to test the agent with an AnotherAI experiment.
```
...
```
Can you retry the last 5 completions of anotherai/agent/food-analyzer and compare the outputs with
GPT 5 mini?
```

![Model comparison experiment](/images/model-comparison-experiment-food-analyzer.png)
...
```
Test my food-analyzer agent with temperatures 0.2, 0.5, and 0.8 to find
the right balance between creativity and professionalism
```

![Food Analyzer Temperature Experiment](/images/temperature-experiment-food-analyzer.png)
...
<Callout type="info">
Images can currently only be processed as input to agents. Image generation as output is not yet supported.
</Callout>

## Debugging Image Agents

The process of debugging image agents is the same as debugging text-based agents. You can learn more about debugging agents in our [Debugging guide](/use-cases/fundamentals/debugging).
