...
```
Create a new AnotherAI agent that provides a description for a given image
```
...
```
Create a new AnotherAI agent that provides a description for a given image where the average response time is less than 4 seconds
```

**For high-volume agents that need to be cost-effective:**

```
Create a new AnotherAI agent that provides a description for a given image that costs less than $5 for 1000 requests
```
...
```
Create a new AnotherAI agent that provides a description for a given image and include a customer_id metadata key.
```
...
```
How much is anotherai/agent/image-description-generator costing me this month?
```
...
```
Look at the prompt of anotherai/agent/image-description-generator and create an experiment in AnotherAI 
that compares the current prompt with a new prompt that better emphasizes direct over flowery descriptions.
```
...
```
Create an AnotherAI experiment to help me find a faster model for anotherai/agent/image-description-generator,
but still maintains the clear and concise description style as my current model.
```
...
```
Can you retry the last 5 completions of anotherai/agent/image-description-generator and compare the outputs with
GPT 5 mini?
```
...
```
Test my image-description-generator agent with temperatures 0.2, 0.5, and 0.8 to find
