...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie count from a given image
```
...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie count from a given imagewhere the average response time is less than 4 seconds
```

**For high-volume agents that need to be cost-effective:**

```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie count from a given image that costs less than $5 for 1000 requests
```
...
```
Create a new AnotherAI agent that provides a list of detected foods and their approximate calorie count from a given image and include a customer_id metadata key.
```
...
For text-based agents, it's quite simple for your AI assistant to generate data for you, however for image-based agents, AI assistants are inconsistent about generating valid image URLs as input. Therefore, to produce useful results, you will need to use your own images for testing. There a couple ways to do this:

#### Using a Dataset

A dataset is a collection of images in a file format that your AI assistant can understand. Datasets can contain expected outputs, but do not need to. 

Datasets can be useful because they allow you to make a large number of image inputs easily available in one file. If working in an IDE, you can commit the dataset file to your codebase to make it accessible to your team. If working with an AI assistant like ChatGPT, having a dataset means you only need to upload one file instead of each image individually.
...
[
  {
    ...
    "expected_description_elements": ["forest", "mountain", "lake"]
  },
  {
    "image_url": "https://example.com/images/photo2.jpg",
    "expected_description_elements": ["black cat", "green eyes", "blue background"]
  },
  {
    "image_url": "https://example.com/images/photo3.jpg",
    "expected_description_elements": ["salad", "white bowl", "table", "silverware"]
  }
]
```
...
- **Claude Code:** When giving Claude Code images for experiments, it's important to reference them using their full file path (e.g., `/Users/username/Pictures/photo.jpg`) rather than dragging and dropping into the chat. Dragged and dropped images do not contain a file path, which is required for Claude Code to use the image in an AnotherAI experiment.
- **Cursor:** Similarly, when working with Cursor's AI assistant, to add images to AnotherAI experiments, you must provide the full file path (e.g., `/Users/username/Pictures/photo.jpg`) so the assistant can upload the file to AnotherAI's storage.

Example prompt with proper file path reference:
```
Add these images as input to anotherai/experiment/01999c4c-cb00-72bf-1b1b-7096ae019363:
- /Users/username/Pictures/photo1.jpg
- /Users/username/Pictures/photo2.jpg
- /Users/username/Pictures/photo3.jpg
```
</Callout>
...
that compares the current prompt with a new prompt that ensures the food is listed from largest to smallest detected quantity.
```
...
but still maintains the clear and accurate analysis as my current model.
```
