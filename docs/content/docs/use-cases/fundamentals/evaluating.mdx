...
The following process is adapated from Shreya Shankar and Hamel Husain's AI evaluation process. To learn more, see [this podcast](https://www.youtube.com/watch?v=BsWxPI9UM4c).

**What is a Failure Mode?**

A failure mode is specific category of error that your agent is prone to making. Identifying specific failure modes allows you to build evalation scripts that handle evaluating your agent's outputs based on a single scope. Writing an evaluation (either a script or an LLM-as-a-judge) that handles a single failure mode is much easier than writing one that handles all failure modes.
...
![Extracted Open Codes from Annotations](/images/extracted-open-codes-annotations.png)
...
- Severe failure modes: these are failures that have the largest impact on your user's experience with your agent. These might be aspects of your agent's outputs where accuracy is essential (ex. an output field returning whether an input food product contains an allergen), or failure modes that occur frequently enough to a significate number of individual users are impacted.
- Subjective, nuanced failure modes: these are failures modes where there isn't one, correct answer. For example, if evaluating responses of a chatbot agent, there isn't always a single, correct answer to when the chatboth should connect the user with a live customer support agent.

If you want to create a process for regularly evaluating the agent's performance on a given failure mode, keep reading to get more details on your possible evaluation approaches!
