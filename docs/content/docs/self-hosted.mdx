...
```bash
Create an experiment in AnotherAI that compares how GPT-5 mini and GPT-4 mini perform on anotherai/agent/[your-agent-name-here].
```
```bash
Use AnotherAI to help me find a faster model for anotherai/agent/[your-agent-name-here] while still maintaining accuracy.
```
