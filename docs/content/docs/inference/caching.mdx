...
completion = client.chat.completions.create(
  model="anotherai/travel-assistant/production#1", # Using an AnotherAI deployment
...
completion1 = client.chat.completions.create(
  model="anotherai/customer-support/production#1",
...
Or query directly using SQL:

```sql
-- Query cache performance via MCP
query_completions SELECT 
  agent_id,
  COUNT(*) as total_requests,
  SUM(CASE WHEN cost_usd = 0 THEN 1 ELSE 0 END) as cache_hits,
  ROUND(SUM(CASE WHEN cost_usd = 0 THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) as cache_hit_rate
FROM completions 
