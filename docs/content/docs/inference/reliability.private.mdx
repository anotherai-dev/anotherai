...
- `{{API_URL}}/v1/chat/completions` (compatible with OpenAI SDKs)
- `{{API_URL}}/v1/[org-id]/tasks/[agent-id]/schemas/[schema-id]/run` (for AnotherAI SDKs)
...
You can access the AnotherAI status page via RSS feed at:

**RSS Feed URL:** `https://status.anotherai.com/feed.rss`
...
<Callout type="info">
The exhaustive fallback definitions are visible in the [codebase](https://github.com/AnotherAI/AnotherAI/blob/main/api/core/domain/models/model_data_mapping.py) 
</Callout>
...
<Tabs items={["Python", "TypeScript", "cURL"]}>
  <Tab>
  ```python
  @anotherai.agent(id="user-extraction", version="production", use_fallback=["gemini-2.0-flash-001", "o3-mini-latest-medium"])
  ...
  <Tab>
  ```typescript
  const extractUser= anotherAI.agent<UserExtractionInput, UserExtractionOutput>({
    id: 'user-extraction',
    schemaId: 1,
    version: 'production',
  ...
  <Tab>
  ```sh
  curl -X POST {{API_URL}}/v1/agents/user-extraction/schemas/1/run \
  -H "Authorization: Bearer $ANOTHERAI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
      "version": "production",
...
<Callout type="info">
If you have any questions about our architecture, please [contact us](mailto:team@anotherai.support).
</Callout>
