...
Or query completion costs directly using SQL:

```sql
query_completions SELECT SUM(cost_usd) as total_cost FROM completions WHERE agent_id = 'my-agent' AND created_at > '2024-01-01'
```
