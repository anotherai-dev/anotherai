...
<Tabs items={['Python (OpenAI SDK)', 'TypeScript (OpenAI SDK)', 'Go (OpenAI SDK)']}>
<Tab>
```python
import openai
...
completion = client.chat.completions.create(  # or client.beta.chat.completions.parse for structured outputs
    model="<agent_id>/..", # prefix model with agent_id to group completions
    messages=[...],
    metadata={"key": "value"}, # user provided metadata
    extra_body={
...
> Prefixing the model with an `agent_id` is a good practice to group completions for a given agent.
...
> Only for libraries that do not give access to the model, it is possible to pass the `agent_id` in the `metadata` parameter.
