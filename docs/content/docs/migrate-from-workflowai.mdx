...
A migrated deployment will have the shape: `<agent_id>/<deployment_deployment_environment>#<deployment_number>`

In the example above, the migrated deployment will have the shape: `analyze-book-characters/production#1`
...
export async function analyzeBookCharacters(input: AnalyzeBookCharactersTaskInput): Promise<AnalyzeBookCharactersTaskOutput> {
    const completion = await client.chat.completions.create({
        model: "analyze-book-characters/production#1", // use the deployment id here
...
```json
{
    ...
    "model": "analyze-book-characters/production#1",
    "messages": [],
...
If the agent uses a deployment, you will need to adjust the `deployment_id` to match the imported one.
