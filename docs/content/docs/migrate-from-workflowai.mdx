...
<Tabs groupId="framework" items={['TypeScript (OpenAI SDK)',  'Go (OpenAI SDK)', 'API', 'TypeScript (Vercel AI)']}>
<Tab value="TypeScript (OpenAI SDK)">
...
AnotherAI is accessible in the Vercel AI through an OpenAI integration. Important considerations:
- The Vercel AI SDK is very restrictive on what fields are allowed in the request body, so most WorkflowAI specific fields are passed through the metadata instead.
- Empty prompt and messages are not allowed by the Vercel AI SDK, so it is needed to pass a prompt with a "-" which will be ignored by AnotherAI.
...
const { object } = await generateObject({
  model: openai(
    "anotherai/deployment/analyze-book-characters:production#1"
  ),
  prompt: "-", // the agent sdk requires a system prompt
  schema: z.object({
    characters: z.array(z.object({
  ...
  providerOptions: {
    openai: {
      metadata: {
        ...
        // Pass the input here instead of the root since VercelAI removes custom fields
        input: {
          book_title: "The Shadow of the Wind",
        },
      },
