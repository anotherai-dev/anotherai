...
We've been burned by vendor lock-in and want to make sure our users are never subject to it. You can try AnotherAI risk-free. If it doesn't provide value, switching back takes minutes, not months.


To switch away from AnotherAI:
- Remove `base_url="{{API_URL}}/v1"`
- If you're not using deployments: your code stays identical
- If you are using deployments: simply ask Claude Code (or your preferred coding agent) to fetch your deployment configuration to recreate the equivalent configuration in your code.
