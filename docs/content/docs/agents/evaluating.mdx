...
client = AsyncOpenAI(
    base_url="{{API_URL}}/v1",  # Automatically replaced with your AnotherAI endpoint
...
async def process_email(email, model, client, experiment_id):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "{{API_URL}}/v1/annotations",  # Automatically replaced with your endpoint
...
async def main():
    ...
    client = AsyncOpenAI(
        base_url="{{API_URL}}/v1",  # Automatically replaced with your endpoint
        api_key=os.environ.get("ANOTHERAI_API_KEY")
    )
...
completions from my email-summarizer agent. Use this completion query:
SELECT * FROM completions WHERE metadata.agent_id = 'email-summarizer' 
AND created_at > now() - INTERVAL 1 DAY ORDER BY created_at DESC LIMIT 50
```
...
```python
# Query recent completions from AnotherAI
import httpx
import os

async with httpx.AsyncClient() as client:
    response = await client.post(
        "{{API_URL}}/v1/query/completions",  # Automatically replaced with your endpoint
        json={
            "query": """
