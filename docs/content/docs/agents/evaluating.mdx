    ...
    return await client.chat.completions.create(
        ...
        response_format={
            ...
            "json_schema": {
                ...
                "schema": SummaryEvaluation.model_json_schema()
            }
        }
    )
```
...
async def evaluate_email_summaries(emails, models, client):
    ...
    async def process_email(email, model):
        ...
        try:
            ...
            for criterion in ['completeness', 'accuracy', 'clarity', 
                ...
                score_data = evaluation[criterion]
                annotations.append({
                    "target": {
...
To evaluate production data, modify the code from Step 3 (Run the Evaluation Pipeline) to create an experiment with a completions query instead of loading a static dataset:

```python
# Create experiment using recent production completions
experiment_response = await client.post(
    ...
    json={
        ...
        "completions_query": """
            ...
            WHERE metadata.agent_id = 'email-summarizer' 
...
# The experiment now contains recent completions
# Run the judge on these completions
for completion in experiment_response.json()['completions']:
    # Extract the email content and summary
    email_content = completion['input']['email_content']
    summary = completion['output']
    
    # Run judge evaluation
    eval_response = await create_summary_judge(client, email_content, summary)
    evaluation = json.loads(eval_response.choices[0].message.content)
    
    # Store evaluation as annotations...
```
