...
1. Make sure you have the AnotherAI MCP configured and enabled
2. Then just ask Claude Code to set up experiments for you. 
...
You can ask Claude some like as follows:
...
Different models excel at different tasks. Experiments help you find the right balance between intelligence, cost, and speed.

AnotherAI supports over 100 different models, and Claude Code can help you choose the right model for your task, based on what you're looking for.
...
If you have a specific model in mind that you want to try (for example, a newly release models), you can ask Claude directly to help you test that model. You can always request that Claude use inputs from existing completions, to ensure that you're testing with real, production data. 

For example:

```
Can you retry the last 5 completions of @email-rewriter.py and compare the outputs with GPT 5 mini?
```
...
```
Test my email-rewriter agent with temperatures 0.2, 0.5, and 0.8
to find the right balance between creativity and professionalism
```
...
anyacherniss marked this conversation as resolved.
