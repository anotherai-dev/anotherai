...
The first step is to install the MCP server, you can find the instructions [here](/getting-started). Once the MCP server is installed, find your first use-case by looking at our use-cases in the [docs](/use-cases). New accounts will get 1$ of free credits to try it out.
