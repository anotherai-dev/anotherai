...
interface EditViewTooltipProps {
  viewId: string;
}

export function EditViewTooltip({ viewId }: EditViewTooltipProps) {
  const { showToast } = useToast();

  const promptText = `Can you update anotherai/view/${viewId} to `;
  ...
  return (
    <div className="max-w-xs py-1.5 px-1">
      <div className="text-xs font-medium text-white">
        ...
        <div className="mt-3 italic font-bold w-full whitespace-pre-wrap">{`Can you update anotherai/view/${viewId} to...[your changes here]`}</div>
      </div>
