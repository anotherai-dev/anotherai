...
export function SchemaViewer(props: SchemaViewerProps) {
  ...
  const renderProperty = (
    ...
    const isShared =
      !sharedKeypathsOfSchemas || sharedKeypathsOfSchemas.includes(currentPath);
    ...
    if ((node.type === "object" || node.properties) && node.properties) {
      ...
      return (
        <div
          key={key}
          className="border border-gray-200 rounded-[2px] mb-3 w-fit"
        >
          ...
          <div className="bg-transparent">
            {Object.entries(node.properties).map(
              ([childKey, childNode], index) => (
                <div key={childKey}>
                  {index > 0 && (
                    <div className="border-t border-dashed border-gray-200 -mx-px"></div>
                  )}
                  <div className="p-3">
                    {renderProperty(childKey, childNode, currentPath)}
                  </div>
                </div>
              )
            )}
    ...
    if ((node.type === "array" || node.items) && node.items) {
      const fullKeyPath = annotationPrefix
        ? `${annotationPrefix}.${currentPath}`
        : currentPath;
      return (
        <div key={key}>
          <div className="py-1">
            <HoverContainer
              ...
              onKeypathSelect={
                onKeypathSelect ? () => onKeypathSelect(currentPath) : undefined
              }
            >
          ...
          <div className="ml-4 mt-2 relative">
            ...
            <div className="ml-3 mt-2">
              {renderProperty("", node.items, currentPath)}
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div
      className={`border border-gray-200 rounded-[2px] bg-white ${className}`}
    >
      <div className="p-3 max-h-80 overflow-y-auto">
        ...
        {jsonSchema.properties ? (
          <div className="space-y-1">
            {Object.entries(jsonSchema.properties).map(([key, node]) =>
              renderProperty(key, node, "")
            )}
          </div>
        ) : (
          <div className="text-xs text-gray-500 italic font-normal">
