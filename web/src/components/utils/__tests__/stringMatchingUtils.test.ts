...
describe("findCommonSubstrings", () => {
  describe("Input validation", () => {
    ...
    it("returns empty string for null/undefined input", () => {
      expect(findCommonSubstrings(null)).toBe("");
      expect(findCommonSubstrings(undefined)).toBe("");
    });

    it("filters out null/undefined/short texts", () => {
      expect(findCommonSubstrings([null, undefined, "ab", "test text"])).toBe("test text");
    });
  ...
  describe("Basic substring matching", () => {
    ...
    it("finds multiple common substrings", () => {
      const texts = ["The quick brown fox jumps", "The quick red fox runs", "The quick blue fox walks"];
  ...
  describe("Advanced pattern matching", () => {
    ...
    it("prioritizes longer matches over shorter ones", () => {
      const texts = ["machine learning algorithm", "machine learning model", "machine learning system"];
      const result = findCommonSubstrings(texts);
  ...
  describe("Edge cases and whitespace handling", () => {
    ...
    it("skips whitespace-heavy substrings", () => {
      const texts = ["word1     word2", "word1     word3", "word1     word4"];
      const result = findCommonSubstrings(texts);
  ...
  describe("Performance optimizations", () => {
    it("uses optimized approach for large datasets", () => {
      const largeTexts = Array(15)
        .fill(0)
        .map((_, i) => `This is test number ${i} with common patterns and shared vocabulary elements`);
  ...
  describe("Word-based matching in optimized approach", () => {
    ...
    it("preserves original casing", () => {
      const texts = ["OpenAI GPT Model", "OpenAI BERT Model", "OpenAI T5 Model"];
      const result = findCommonSubstrings(texts);
    ...
    it("handles mixed case correctly", () => {
      const texts = ["JavaScript is great", "javascript is powerful", "JavaScript is useful"];
      const result = findCommonSubstrings(texts);
      expect(result.length).toBeGreaterThan(0);
    });
  });
  ...
  describe("Fallback behavior", () => {
    it("falls back to character-based matching when no common words exist", () => {
      const texts = ["abc123def456", "xyz123ghi456", "rst123jkl456"];
      const result = findCommonSubstrings(texts);
    ...
    it("handles emoji and unicode characters", () => {
      const texts = ["Hello 👋 world 🌍", "Hello 👋 universe 🌌", "Hello 👋 earth 🌎"];
      const result = findCommonSubstrings(texts);
      expect(result).toContain("Hello");
    });
  });
  ...
  describe("Result formatting and limits", () => {
    it("limits result length appropriately", () => {
      const texts = Array(10)
        .fill(0)
        .map((_, i) => `common word${i} shared phrase${i} repeated pattern${i} test data${i}`);
      const result = findCommonSubstrings(texts);
      expect(result.length).toBeLessThan(500); // Reasonable length limit
    });

    it("joins results with spaces", () => {
      const texts = ["first common second common", "first shared second shared", "first pattern second pattern"];
      const result = findCommonSubstrings(texts);
