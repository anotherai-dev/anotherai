  ...
  return (
    <div className="relative group">
      <div
        ...
        className={`flex items-center rounded-[2px] text-xs relative cursor-move ${isDragging ? "opacity-50" : ""} ${
          isActive()
            ? "bg-blue-100 text-blue-700"
            : `text-gray-700 hover:bg-gray-100 ${isMenuOpen ? "bg-gray-100" : ""}`
        }`}
      >
        <button onClick={handleViewClick} className="flex items-start gap-1.5 pl-7 pr-3 py-2 flex-1 min-w-0 text-left">
          <div className="flex-shrink-0 mt-0.5">
            {view.view_type === "run_list" ? <Table className="w-3 h-3" /> : <BarChart3 className="w-3 h-3" />}
          </div>
          <EditableViewTitle
            ref={viewTitleRef}
            viewId={view.id}
