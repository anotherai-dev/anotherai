...
export const useAgentsWithCounts = create<AgentsWithCountsState>((set, get) => ({
  agents: [],
  ...
  fetchAgentsWithCounts: async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch agents with counts: ${response.status} ${response.statusText}`);
      }

      const result = await response.json();
      const agents: AgentWithCompletionCount[] = (Array.isArray(result) ? result : result.data || []).map(
        (item: Record<string, unknown>) => ({
          agent_id: String(item.agent_id),
          completion_count: Number(item.completion_count || item["COUNT(*) as completion_count"] || 0),
        })
      );
...
export const useOrFetchAgentsWithCounts = () => {
  const fetchAgentsWithCounts = useAgentsWithCounts((state) => state.fetchAgentsWithCounts);
