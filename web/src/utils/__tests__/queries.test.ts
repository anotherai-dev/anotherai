import { defaultQueryParts, buildQuery, defaultQuery } from '../queries';
...
describe('Query Building Utilities', () => {
  ...
  describe('buildQuery', () => {
    ...
    it('handles null whereClause', () => {
      const result = buildQuery(null as any);
    ...
    it('handles complex WHERE clauses', () => {
      const complexWhere = "agent_id IN ('agent1', 'agent2') AND created_at >= '2023-01-01' AND (cost_usd > 0.1 OR duration_ds > 1000)";
  ...
  describe('Performance and edge cases', () => {
    it('handles very long WHERE clauses', () => {
      const longConditions = Array(50).fill(0).map((_, i) => `field${i} = 'value${i}'`).join(' AND ');
